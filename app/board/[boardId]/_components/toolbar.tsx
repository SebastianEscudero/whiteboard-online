...
export const Toolbar = ({
  ...
  return (
    <div className="absolute bottom-2 left-[50%] translate-x-[-50%] flex md:flex-row flex-col gap-x-4">
      ...
      <div className="bg-white dark:bg-[#1A1A1C] rounded-lg shadow-custom-3 p-1.5 flex gap-x-1 flex-row items-center">
        <ToolButton
          label="Select"
          icon={MousePointer2}
      ...
      <div className="bg-white dark:bg-[#1A1A1C] rounded-md p-1.5 hidden md:flex flex-row items-center shadow-custom-3">
      ...
      {isPenMenuOpen && (canvasState.mode === CanvasMode.Highlighter || canvasState.mode === CanvasMode.Pencil) && isPenEraserSwitcherOpen &&
        <div className="p-3 pt-5 pb-2 absolute left-8 bottom-32 bg-white dark:bg-[#1A1A1C] rounded-lg shadow-custom-1 w-[165px] flex flex-col items-center cursor-default">
          <Slider
            defaultValue={[pathStrokeSize]}
            min={1}
            max={8}
            step={1}
            className='bg-white dark:bg-[#1A1A1C] w-[90%] cursor-pointer'
      ...
      {isShapesMenuOpen && canvasState.mode === CanvasMode.Inserting && canvasState.layerType !== LayerType.Image && canvasState.layerType !== LayerType.Text && canvasState.layerType !== LayerType.Arrow && canvasState.layerType !== LayerType.Note &&
        <div className="absolute left-[60px] bottom-16 p-2 bg-white dark:bg-[#1A1A1C] rounded-lg shadow-custom-1 grid grid-cols-4 grid-rows-3 gap-2 h-[144px] w-[160px] sm:w-[192px] items-center cursor-default">
          <ToolButton
            icon={Square}
            onClick={() => setCanvasState({
              mode: CanvasMode.Inserting,
      ...
      {isPenEraserSwitcherOpen && (canvasState.mode === CanvasMode.Pencil || canvasState.mode === CanvasMode.Eraser || canvasState.mode === CanvasMode.Laser || canvasState.mode === CanvasMode.Highlighter) &&
        <div className="absolute left-5 bottom-16 p-2 bg-white dark:bg-[#1A1A1C] rounded-lg shadow-custom-1 flex flex-row space-x-1 items-center cursor-default">
...
export const ToolbarSkeleton = () => {
  return (
    <div className="absolute top-[50%] -translate-y-[50%] left-2 flex flex-col gap-y-4 bg-white dark:bg-[#1A1A1C] h-[360px] w-[52px] shadow-custom-1 rounded-md" />
  );
};